...
def test_ping_wrong_method(api_client, mocker):
    ...
    with pytest.raises(AssertionError, match='Expected 201, but got 405'):
        api_client.ping()
...
def test_ping_internal_server_error(api_client, mocker):
    ...
    mocker.patch.object(api_client.session, 'get', return_value=mock_response)
    with pytest.raises(AssertionError, match='Expected 201, but got 500'):
        api_client.ping()
...
def test_ping_not_found(api_client, mocker):
    ...
    mocker.patch.object(api_client.session, 'get', return_value=mock_response)
    with pytest.raises(AssertionError, match='Expected 201, but got 404'):
        api_client.ping()
...
def test_ping_success_different_code(api_client, mocker):
    ...
    mocker.patch.object(api_client.session, 'get', return_value=mock_response)
    with pytest.raises(AssertionError, match='Expected 201, but got 200'):
        api_client.ping()
